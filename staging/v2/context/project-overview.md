...
A CLI-native pipeline that generates multiple hardware variants of a base microcontroller simulation, compiles firmware for each variant, and validates them through automated wokwi-cli scenarios. The output is a registry of known-good project directories — each ready to be zipped and imported into wokwi.com.

This is the successor to the v1 Puppeteer-based browser automation approach, which was abandoned due to fragility (DOM selector drift, bot detection, no public publish API).

## Goals

1. Generate 3+ distinct hardware variants from a single base template
...
5. Maintain a JSON registry mapping variant IDs to build results, artifact paths, and metadata
...
- Failure isolation: a build failure skips simulation; a simulation failure still records build success
...
7. Each variant directory is a self-contained project that can be zipped and imported into wokwi.com
