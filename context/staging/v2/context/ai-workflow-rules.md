...
Build this project incrementally using spec-driven workflow. Context files define what to build, how to build it, and current state of progress. Always implement against these specs — do not infer or invent behavior from scratch.
...
- Work on one feature unit at time: one script, one variant configuration, or one bugfix.
- Prefer small, verifiable increments over large speculative changes.
- Do not combine unrelated system boundaries in single implementation step.
...
- If requirement is ambiguous, resolve it in relevant context file before implementing.
- If a requirement is missing, add it as an open question in `progress-tracker.md` before continuing.

## Protected Files

Do not modify following unless explicitly instructed:
